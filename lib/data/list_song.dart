import 'package:flutter_mp3/models/SongModel.dart';

class ListSong {
  static final list = [
    SongModel(
        id: '1',
        name: "Chưa Quên Người Yêu Cũ (Live Version)",
        artist: "Hà Nhi",
        audio: "ChuaQuenNguoiYeuCuLiveVersion.mp3",
        lyrics: 'Cũng đã gần ba năm\n'
            'Mà em vẫn nhớ anh nhiều lắm\n'
            'Vẫn chưa thể yêu ai\n'
            'Vẫn chưa nắm tay ai\n'
            'Thế gian này rộng lớn\n'
            'Mà con tim chẳng to nhiều hơn\n'
            'Chỉ vừa đủ nhớ một người\n'
            'Chỉ vừa đủ thương một người\n\n'
            'Đâu phải em muốn quên\n'
            'Là sẽ quên, là sẽ quên\n'
            'Đâu phải mong hết đau\n'
            'Là bớt đau, là bớt đau\n'
            'Nhiều khi lí trí cố gắng mạnh mẽ\n'
            'Nhưng trái tim thật sự yếu đuối\n'
            'Là em giấu đi, chỉ là em giấu đi\n'
            'Đâu phải yêu hết tâm\n'
            'Người để tâm, người để tâm\n'
            'Đâu phải cho hết đi\n'
            'Người chẳng đi, người chẳng đi\n'
            'Chỉ vì đôi khi yêu thương một ai\n'
            'Ta chẳng mong nhận lại chi hết\n'
            'Ngờ đâu mấy khi\n'
            'Tình yêu lại khiến ta hoen bờ mi\n\n'
            'Nhớ khi mình quen nhau\n'
            'Cùng đi trên chiếc xe đạp nâu\n'
            'Lá xanh còn chưa chín\n'
            'Đã hẹn ước đôi mình\n'
            'Ước muôn đời bên nhau\n'
            'Bàn tay nắm mãi chẳng rời đâu\n'
            'Một câu nói trót lỡ lời\n'
            'Rồi ta mất nhau một đời\n\n'
            'Đâu phải em muốn quên\n'
            'Là sẽ quên, là sẽ quên\n'
            'Đâu phải mong hết đau\n'
            'Là bớt đau, là bớt đau\n'
            'Nhiều khi lí trí cố gắng mạnh mẽ\n'
            'Nhưng trái tim thật sự yếu đuối\n'
            'Là em giấu đi, chỉ là em giấu đi\n'
            'Đâu phải yêu hết tâm\n'
            'Người để tâm, người để tâm\n'
            'Đâu phải cho hết đi\n'
            'Người chẳng đi, người chẳng đi\n'
            'Chỉ vì đôi khi yêu thương một ai\n'
            'Ta chẳng mong nhận lại chi hết\n'
            'Mà không thể quên\n'
            'Cũng là một cách yêu thương gọi tên\n\n'
            'Chẳng phải em không thể\n'
            'Không thể tìm được một ai khác\n'
            'Chỉ là vì anh đã đi\n'
            'Để lại khoảng không khó đầy\n'
            'Một mình em vẫn tốt\n'
            'Vẫn tự mình dịu xoa những tổn thương\n'
            'Chờ ngày trái tim em lại yêu\n'
            'Và yêu nhiều hơn nữa\n\n'
            'Đâu phải em muốn quên\n'
            'Là sẽ quên, là sẽ quên\n'
            'Đâu phải yêu hết tâm\n'
            'Người để tâm, người để tâm\n'
            'Đâu phải cho hết đi\n'
            'Người chẳng đi, người chẳng đi\n'
            'Chỉ vì đôi khi yêu thương một ai\n'
            'Ta chẳng mong nhận lại chi hết\n'
            'Ngờ đâu mấy khi\n'
            'Tình yêu lại khiến ta hoen bờ mi\n\n'
            'Và không thể quên\n'
            'Cũng là một cách yêu thương gọi tên\n'),
    SongModel(
        id: '2',
        name: "Túy Âm (Cover)",
        artist: "O Sen",
        audio: "TuyAmOSen.mp3"),
    SongModel(
        id: '3',
        name: "Người Như Anh",
        artist: "Mai Tiến Dũng",
        audio: "NguoiNhuAnh.mp3"),
    SongModel(id: '4', name: "Believe Me", audio: "SteinsGateBelieveMe.mp3"),
    SongModel(
        id: '5', name: "Believe Me Zero", audio: "SteinsGateBelieveMeZero.mp3"),
    SongModel(
        id: '6', name: "Fake Verthandi", audio: "SteinsGateFakeVerthandi.mp3"),
    SongModel(
        id: '7', name: "Gate Of Steiner", audio: "SteinsGateGateOfSteiner.mp3"),
    SongModel(
        id: '8', name: "Logical Theme", audio: "SteinsGateLogicalTheme.mp3"),
    SongModel(id: '9', name: "Lyra", audio: "SteinsGateLyra.mp3"),
    SongModel(id: '10', name: "Messenger", audio: "SteinsGateMessenger.mp3"),
    SongModel(
        id: '11',
        name: "Messenger Main Theme",
        audio: "SteinsGateMessengerMainTheme.mp3"),
    SongModel(id: '12', name: "Re Awake", audio: "SteinsGateReawake.mp3"),
    SongModel(
        id: '13',
        name: "The End Of Messenger",
        audio: "SteinsGateTheEndOfMessenger.mp3"),
  ];
}
